...
exports.list = async(req, res) => {
    try {
        ...
        const totalCount = await Loan.aggregate(countPipeline);
        if((totalCount.length == 0) || (totalCount[0].totalCount == 0)) {
            return res.status(200).json({status:200, message: "No loan list", data: []})   
        }
        const totalPage = Math.ceil(totalCount[0].totalCount/row);
        pipeline.push(filter, lookup, searchFilter, {$skip: offset}, {$limit: row});
        const loanList = await Loan.aggregate(pipeline);
        if(!loanList.length) {
            return res.status(200).json({status:200, message: "No loan list", data: []})   
        }
        return res.status(200).json({status:200, message: "Loan list successfully", data: [{page: page.toString()+" of "+ totalPage.toString(), list:loanList}]}) 
    } catch (error) {
        return res.status(400).json({status:400, message: "Error while getting loan list", data: ""}) 
    }
}
...
exports.add = async(req, res) => {
    try {
        ...
        const loan = await Loan.insertOne(payload);
        if(!loan) {
            return res.status(400).json({status:400, message: "Error while adding employee loan", data: ""}) 
        }
        return res.status(200).json({status:200, message: "Employee loan addedd successfully", data: ""}) 
    } catch (error) {
        return res.status(400).json({status:400, message: "Error while adding employee loan", data: ""}) 
    }
}
...
exports.edit = async(req, res) => {
    try {
        ...
        const loanExist = await Loan.findOne(query);
        if(!loanExist) {
            return res.status(404).json({status:404, message: "Employee loan not found", data: ""}) 
        }
        const payload = {};
        if(req.body.amount) {
        ...
        const loanUpdate = await Loan.updateOne(req.params.loanID, payload);
        if(!loanUpdate.modifiedCount) {
            return res.status(400).json({status:400, message: "Error while updating employee loan", data: []}) 
        }
        return res.status(202).json({status:202, message: "Employee loan updated successfully", data: ""}) 
    } catch (error) {
        return res.status(400).json({status:400, message: "Error while updating employee loan", data: ""}) 
    }
}
...
exports.delete = async(req, res) => {
    try {
        ...
        const loanUpdate = await Loan.deleteOne(req.params.loanID);
        if(!loanUpdate.modifiedCount) {
            return res.status(400).json({status:400, message: "Error while deleteing employee loan", data: []}) 
        }
        return res.status(202).json({status:202, message: "Employee loan deleted successfully", data: ""}) 
    } catch (error) {
        return res.status(400).json({status:400, message: "Error while deleteing employee loan", data: ""}) 
    }
}